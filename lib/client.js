...
function defaultSetup(game, avatar, client) {
  ...
  // blocks
  var blockSelector = toolbar({ el: '#blocks', toolbarKeys: ['t','y','u','i','o','p'] })
  ...
  toolSelector.on('select',function(selection){
    var toolNames = playerTools.map(function(tool){ return tool.name })
    var newTool = playerTools[toolNames.indexOf(selection)]
