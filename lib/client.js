...
Client.prototype.bindClientEvents = function() {
  ...
  baseClient.on('loadComplete', function() {
    ...
    require('./AsciiEffect.js')(THREE)
    effect = new THREE.AsciiEffect( renderer, undefined, {color: false} )
    effect.setSize( view.width, view.height )
    self.settings.container.appendChild( effect.domElement )
    effect.setSize( view.width, view.height )
    setTimeout(function() {
    ...
    function animate() {
      ...
      effect.render( game.scene, view.camera )
    }
